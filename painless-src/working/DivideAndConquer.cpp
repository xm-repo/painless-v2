...
void * mainMasterDivideAndConquer(void * arg)
{
   ...
   log(0, "Master has started worker(s) and is now waiting and managing" \
          " requests.\n");
   ...
   while(globalEnding == false && dc->strategyEnding == false) {
      ...
      while(dc->overs.empty() ||
            (dc->workers.empty() && dc->workQueue->empty())){
         log(1, "Thread Master is waiting for work.\n");
         log(1, "DBG: overs-size: %d, workers-size: %d, workQ-size: %d)\n",
                dc->overs.size(), dc->workers.size(), dc->workQueue->size());
      ...
      if (successGetJob) {
         ...
         log(1, "A job has been taken from the workqueue\n");
         ...
         if(delta > 0) {
            log(1, "Master: workers are working for too short, I will " \
                   "sleep %f sec\n", delta);
         ...
         log(1,"Master Interrupts %p and added %p to splitting list\n",
...
{
   if (strategyEnding == true || globalEnding == true)
      return;
   ...
   if (res == UNKNOWN){
      ...
      if(cloneStrategy == 2) {
         ((SequentialWorker *)over)->solver = NULL;

         if (overSolver != NULL) {
            if (sharers != NULL) {
               for (int i = 0; i < nSharers; i++) {
                  sharers[i]->removeConsumer(overSolver);
                  sharers[i]->removeProducer(overSolver);
               }
            }
            overSolver->release();
         }
      ...
      if(!overs.empty()) {
         // Signal the Master that some workers need jobs
   ...
   if(res == UNSAT) {
      ...
      if(doesVectorContain(workersSplitting, current)) {
         log(1, "Slave %p didn't split because it has finished\n", current);
   ...
   log(1, "DBG: The search is about to end. Post conditions: workers-empty: " \
          "%d, workQ-empty: %d\n", workers.empty(), workQueue->empty());
